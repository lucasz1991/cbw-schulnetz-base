...
{
    /** Aktuelle Daten für das Dashboard */
    public array $teilnehmerDaten = [];

    /** Fallback-Daten (Demo/Altbestand) */
    public array $teilnehmerDatenFallback = [
        "teilnehmer" => [
            "teilnehmerNr" => "000007",
    ...
    {
        // Holt JSON/Array aus Auth->person->programdata, oder nutzt Fallback
        $this->teilnehmerDaten = Auth::user()?->person?->programdata ?? $this->teilnehmerDatenFallback;

        // Optional: Sicherstellen, dass es ein Array ist (falls programdata als JSON-String vorliegt)
        if (is_string($this->teilnehmerDaten)) {
            $decoded = json_decode($this->teilnehmerDaten, true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                $this->teilnehmerDaten = $decoded;
            } else {
                $this->teilnehmerDaten = $this->teilnehmerDatenFallback;
            }
        }
    }
